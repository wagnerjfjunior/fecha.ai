...
**Escopo:** varredura documental ampliada de `docs/` para identificar arquivos, domínios e gaps do inventário anterior.  
...
Ampliar o inventário documental do FECH.AI para aproximar a visão da árvore real de `docs/`, identificando arquivos que não apareceram ou não foram detalhados no Inventário Documental v1.

Este documento serve para preparar as próximas auditorias específicas:

1. MesaCliente;
2. Supabase/RPC/RLS;
3. código AS-IS;
4. matriz docs x código x Supabase.

---

## 2. Método de varredura

A varredura foi feita usando busca no repositório GitHub por caminhos e domínios documentais.
...
Limitação importante: esta varredura depende da busca/indexação do GitHub e não substitui um `git ls-tree -r --name-only main docs/` ou export local completo do repositório. Portanto, ela é ampliada, mas ainda deve ser marcada como `PENDENTE_RECONCILIACAO` até uma árvore Git completa ser gerada.

---

## 3. Diretórios documentais identificados

| Diretório | Domínio principal | Status | Observação |
|---|---|---:|---|
| `docs/` | M0 | OFICIAL_CANDIDATO | Entrada geral da documentação. |
| `docs/audits/documentation/` | M0 | OFICIAL_CANDIDATO | Auditorias documentais recém-criadas. |
...
## 4. Arquivos identificados por diretório

### 4.1 Raiz e arquitetura
...
- `arquitetura-atual.md`: OFICIAL_CANDIDATO;
- `mapa-tabelas.md`: PENDENTE_RECONCILIACAO;
- `rpcs-e-functions.md`: PENDENTE_RECONCILIACAO;
- `dicionario-de-dados.md`: PENDENTE_RECONCILIACAO;
- `lgpd.md`: RASCUNHO / PENDENTE_RECONCILIACAO;
- `seguranca-multitenant.md`: OFICIAL_CANDIDATO / PENDENTE_RECONCILIACAO.

### 4.2 Auditoria documental
...
- auditoria documental v1: OFICIAL_CANDIDATO / PENDENTE_RECONCILIACAO;
- inventário documental v1: OFICIAL_CANDIDATO / PENDENTE_RECONCILIACAO;
- este documento: OFICIAL_CANDIDATO / PENDENTE_RECONCILIACAO.

### 4.3 Branches, changelog, produto, roadmap e protocolos
...
- há possível duplicidade entre protocolos universais com nomes hifenizados e underscored;
...
### 4.7 Power Message Engine e PME
...
### 4.8 MesaCliente — núcleo identificado
...
### 4.9 MesaCliente — fases e evidências identificadas
...
## 5. Deltas relevantes contra o Inventário Documental v1

O Inventário Documental v1 já era útil, mas esta varredura ampliada identificou categorias adicionais ou mais detalhadas:
...
| Skills GPT 0 e GPTs 7-10 agora existem no registry, mas code search pode demorar a indexar | Knowledge dos GPTs deve ser tratado como cache auxiliar | P2 |
...
4. Criar auditoria de código AS-IS:
