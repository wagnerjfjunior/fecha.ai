...
This section becomes the current semantic next action when this bounded STS-M2-02 acceptance reconciliation is merged to canonical `main`.
...
→ bootstrap current Backend/Data + Architecture routing; add AppSec if security significance is material
→ inventory current indexes and usage/statistics evidence required by the task
→ enumerate contradictory/redundant ACL candidates carried from STS-M2-02
→ distinguish current effective access from latent grant/policy combinations
→ produce bounded STS-M2-03 evidence/acceptance package

NO DDL/DML
NO migration
NO index mutation
NO grant/revoke
NO RLS/policy/RPC/trigger mutation
NO Auth/Edge mutation
NO deploy
NO Security Go
```

Do not reopen STS-M2-01, the 160-routine universe, the 137 SECURITY DEFINER inventory, the resolved `59/57/56/8` metrics or the `137/137` static caller-provenance closure absent material invalidation.
...
STS-M2-01 = COMPLETE / ACCEPTED
STS-M2-02 = NEXT
STS-M2-02 implementation = NOT_AUTHORIZED
Security Go = NOT_GRANTED
```
...
```text
STS-M2-02 — MAPA DE ROUTINES / POLICIES / TRIGGERS / GRANTS — READ_ONLY

→ resolve FECH.AI main live
→ bootstrap current Backend/Data + Architecture routing; add AppSec where security adjudication is material
→ enumerate current routines / policies / triggers / grants
→ map owner, caller, tenant authority and write authority
→ distinguish catalog evidence from runtime behavior
→ identify contradictions/gaps without mutating live state
→ produce bounded M2-02 evidence/acceptance package

NO DDL/DML
NO migration
NO policy/RLS/grant/RPC/trigger mutation
NO Auth/Edge mutation
NO deploy
NO Security Go
```
...
M2 BOOTSTRAP
→ resolve live main
→ read canonical bootstrap / governance / SFJM
→ reconstruct exact M2 milestone contract
→ bound M2-01 scope and evidence needs
→ no implementation before separate Product Authority authorization
```

M2 is eligible but not started by this reconciliation. Do not reopen M1 technical work absent a
material contradiction or the deferred-evidence reopen trigger.
