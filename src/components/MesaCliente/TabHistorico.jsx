...
function HistCard({ item, isGestor, empresaId, onRefetch }) {
  ...
  return (
    <div className="bg-[var(--color-background-primary)] border border-[var(--color-border-tertiary)]
      rounded-2xl overflow-hidden mb-2 hover:border-[var(--color-border-secondary)] transition-colors">
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <div className="text-[12px] text-[var(--color-text-secondary)] mt-0.5">
                {item.empreendimento} {item.unidade ? `· ${item.unidade}` : ''}
              </div>
              <div className="flex gap-3 mt-1.5 flex-wrap">
                ...
                <span className="text-[11px] text-[var(--color-text-tertiary)]">
                  📅 {fmtData(item.atualizado_em)}
                </span>
                {item.tabela_provisoria && (
                  <span className="text-[10px] bg-[#FAEEDA] text-[#412402] px-1.5 py-0.5 rounded">
                    tabela provisória
                  </span>
                )}
              </div>
            ...
            <div className="text-right flex-shrink-0">
              ...
              <span className={`text-[11px] font-medium px-2 py-0.5 rounded mt-1 inline-block ${st.bg} ${st.text}`}>
                {st.label}
              </span>
            </div>
          </div>

          {/* Botão aprovar/rejeitar (gestor) */}
          {podeAprovar && !showAprov && (
            <button
              onClick={() => setShowAprov(true)}
              className="mt-2 text-[11px] px-3 py-1.5 rounded-xl bg-[#FAEEDA] text-[#412402] font-medium"
            >
              Aprovar ou rejeitar →
            </button>
          )}
      ...
      {showAprov && (
        <div className="px-3 pb-3 border-t border-[var(--color-border-tertiary)]">
          <p className="text-[12px] font-medium text-[var(--color-text-primary)] mt-2 mb-2">
          ...
          <textarea
            ...
            className="w-full text-[12px] p-2 rounded-xl border border-[var(--color-border-secondary)]
              bg-[var(--color-background-secondary)] resize-none outline-none"
          />
          <div className="flex gap-2 mt-2">
            <button
              onClick={() => setShowAprov(false)}
              className="flex-1 py-1.5 rounded-xl text-[12px] bg-[var(--color-background-secondary)]"
            >
              Cancelar
            </button>
            <button
              ...
              disabled={isPending}
              className="flex-1 py-1.5 rounded-xl text-[12px] bg-[#FDEAEA] text-[#993556] font-medium"
            >
              Rejeitar
            </button>
            <button
              ...
              disabled={isPending}
              className="flex-1 py-1.5 rounded-xl text-[12px] bg-[#E1F5EE] text-[#0F6E56] font-medium"
            >
              {isPending ? '…' : 'Aprovar'}
            </button>
          </div>
...
export default function TabHistorico({ empresaId, corretorId, isGestor }) {
  ...
  const [empFiltro, setEmp]   = useState('');
  const [stFiltro, setSt]     = useState('');

  const filtros = {
    busca:  busca  || null,
    empId:  empFiltro || null,
  ...
  const {
    ...
    isError,
    refetch,
  } = useHistoricoMesas(empresaId, filtros, corretorId);
    ...
    <div className="p-3">
      ...
      <div className="flex gap-2 mb-3 flex-wrap">
        <input
          ...
          className="flex-1 min-w-[160px] text-[13px] px-3 py-2 rounded-xl
            border border-[var(--color-border-secondary)] bg-[var(--color-background-primary)] outline-none"
        />
        <select
          value={stFiltro}
          onChange={e => setSt(e.target.value)}
          className="text-[13px] px-3 py-2 rounded-xl border border-[var(--color-border-secondary)]
            bg-[var(--color-background-primary)] outline-none"
        >
      ...
      {isError && (
        <div className="text-center py-8">
          <p className="text-[var(--color-text-danger)] text-[13px] mb-3">Erro ao carregar.</p>
          <button onClick={refetch}
            className="text-[12px] px-4 py-2 rounded-xl bg-[var(--color-background-secondary)]">
            Tentar novamente
          </button>
        </div>
      )}
      ...
      {!isLoading && !isError && historico.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-[12px] text-[var(--color-text-tertiary)] mt-1">
            {busca || stFiltro ? 'Tente outros filtros' : 'As propostas salvas na Mesa aparecem aqui'}
          </p>
        </div>
      )}
