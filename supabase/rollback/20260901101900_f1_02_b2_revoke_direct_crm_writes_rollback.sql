-- FECH.AI — F1-02 / B2 rollback
-- Restores only the three authenticated direct CRM grants removed by B2.
--
-- SECURITY WARNING:
-- A successful rollback intentionally reopens F1-02/B2.
-- SECURITY_GO remains DENIED.
--
-- VERSIONED ARTIFACT ONLY.
-- This file is NOT authorization to execute the rollback in Supabase.
...
do $f1_02_b2_rollback_preflight$
declare
  v_count integer;
  v_hash text;
  v_owner text;
...
begin
  if pg_catalog.to_regrole('authenticated') is null
    ...
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_REQUIRED_ROLE_MISSING';
  end if;
  ...
  if pg_catalog.to_regclass('public.leads') is null
    ...
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_REQUIRED_TABLE_MISSING';
  end if;
  ...
  loop
    ...
    if v_owner is distinct from 'postgres'
      ...
      raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_TABLE_INVARIANT_DRIFT table=%',v_sig;
    end if;
  end loop;

  -- Hardened state must be the exact semantic starting point.
  if not pg_catalog.has_table_privilege('authenticated','public.leads','SELECT')
    ...
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_LEADS_NOT_HARDENED';
  end if;
  ...
  if not pg_catalog.has_table_privilege('authenticated','public.lotes','SELECT')
    ...
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_LOTES_NOT_HARDENED';
  end if;
  ...
  if not pg_catalog.has_table_privilege('authenticated','public.times','SELECT')
    ...
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_TIMES_DRIFT';
  end if;
  ...
  if v_hash is distinct from 'b737d3eae6acb29657f866f2f8993734' then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_TIMES_RELACL_DRIFT hash=%',v_hash;
  end if;
  ...
  if v_count <> 0 then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_COLUMN_ACL_PRESENT count=%',v_count;
  end if;
  ...
  if pg_catalog.has_table_privilege('anon','public.leads','INSERT')
    ...
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_ANON_WRITE_PRESENT';
  end if;
  ...
  loop
    if not pg_catalog.has_table_privilege('service_role',v_sig,'SELECT')
      ...
      raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_PRIVILEGED_ROLE_DRIFT table=%',v_sig;
    end if;
  end loop;

  -- Structural fingerprints must still match the reviewed B2 baseline.
  select count(*),
  ...
  if v_count<>3 or v_hash is distinct from '360bc33f4dd17937172e98da5203a220' then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_LEADS_POLICY_DRIFT';
  end if;
  ...
  if v_count<>2 or v_hash is distinct from 'ac27dc669f8136de8004e03ff3ba2276' then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_LOTES_POLICY_DRIFT';
  end if;
  ...
  if v_count<>3 or v_hash is distinct from '20cd727acb7a0b9475e0689949e26f84' then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_TIMES_POLICY_DRIFT';
  end if;
  ...
  if v_count<>2 or v_hash is distinct from 'f1461871ed8742cda6091953c42159e7' then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_LEADS_TRIGGER_DRIFT';
  end if;
  ...
  if v_count<>0 or v_hash is distinct from 'd41d8cd98f00b204e9800998ecf8427e' then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_LOTES_TRIGGER_DRIFT';
  end if;
  ...
  if v_count<>2 or v_hash is distinct from '3b96724deabeb92d0eedd53e617d5625' then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_TIMES_TRIGGER_DRIFT';
  end if;
  ...
  if v_count<>13 or v_hash is distinct from '3fc58a96da3b1235dc5af1cbb5f49a57' then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_LEADS_FK_DRIFT';
  end if;
  ...
  if v_count<>4 or v_hash is distinct from '69573308b1550d317555724642bb6365' then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_LOTES_FK_DRIFT';
  end if;
  ...
  if v_count<>2 or v_hash is distinct from 'ff8a1effa1eb4df4f52361ab3dc3d651' then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_TIMES_FK_DRIFT';
  end if;
  ...
  loop
    ...
    if v_oid is null then
      raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_WRITER_MISSING signature=%',v_sig;
    end if;
    ...
    if v_owner is distinct from 'postgres'
      ...
      raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_WRITER_DRIFT signature=%',v_sig;
    end if;
  end loop;

  v_oid:=pg_catalog.to_regprocedure('public.gerenciar_lista(uuid,text,text)');
  if v_oid is null then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_GERENCIAR_LISTA_MISSING';
  end if;
  ...
  if v_owner is distinct from 'postgres'
     ...
     or pg_catalog.has_function_privilege('anon',v_oid,'EXECUTE')
     or pg_catalog.has_function_privilege('public',v_oid,'EXECUTE') then
    raise exception 'F1_02_B2_ROLLBACK_PREFLIGHT_GERENCIAR_LISTA_DRIFT';
  end if;
  ...
  perform pg_catalog.set_config('fechai.f1_02_b2_rollback_leads_rows',v_rows_leads::text,true);
  perform pg_catalog.set_config('fechai.f1_02_b2_rollback_lotes_rows',v_rows_lotes::text,true);
  perform pg_catalog.set_config('fechai.f1_02_b2_rollback_times_rows',v_rows_times::text,true);
end;
$f1_02_b2_rollback_preflight$;
...
do $f1_02_b2_rollback_postflight$
declare
  v_count integer;
  v_hash text;
...
begin
  if not pg_catalog.has_table_privilege('authenticated','public.leads','SELECT')
    ...
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_LEADS_RESTORE_FAILED';
  end if;
  ...
  if not pg_catalog.has_table_privilege('authenticated','public.lotes','SELECT')
    ...
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_LOTES_RESTORE_FAILED';
  end if;

  if not pg_catalog.has_table_privilege('authenticated','public.times','SELECT')
     or pg_catalog.has_table_privilege('authenticated','public.times','INSERT')
     or pg_catalog.has_table_privilege('authenticated','public.times','UPDATE')
     or pg_catalog.has_table_privilege('authenticated','public.times','DELETE') then
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_TIMES_DRIFT';
  end if;

  -- Exact original table ACL fingerprints must be restored.
  select pg_catalog.md5(coalesce(c.relacl::text,''))
  ...
  if v_hash is distinct from '08a95c631129535b0e07e9be92672b71' then
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_LEADS_RELACL_NOT_RESTORED hash=%',v_hash;
  end if;
  ...
  if v_hash is distinct from 'afa3a93809a23f744356971cbc461855' then
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_LOTES_RELACL_NOT_RESTORED hash=%',v_hash;
  end if;
  ...
  where c.oid='public.times'::pg_catalog.regclass;
  if v_hash is distinct from 'b737d3eae6acb29657f866f2f8993734' then
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_TIMES_RELACL_DRIFT hash=%',v_hash;
  end if;
  ...
  where p.schemaname='public' and p.tablename='leads';
  if v_count<>3 or v_hash is distinct from '360bc33f4dd17937172e98da5203a220' then
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_LEADS_POLICY_DRIFT';
  end if;
  ...
  where p.schemaname='public' and p.tablename='lotes';
  if v_count<>2 or v_hash is distinct from 'ac27dc669f8136de8004e03ff3ba2276' then
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_LOTES_POLICY_DRIFT';
  end if;
  ...
  where p.schemaname='public' and p.tablename='times';
  if v_count<>3 or v_hash is distinct from '20cd727acb7a0b9475e0689949e26f84' then
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_TIMES_POLICY_DRIFT';
  end if;
  ...
  loop
    v_oid:=pg_catalog.to_regprocedure(v_sig);
    if v_oid is null then
      raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_WRITER_MISSING signature=%',v_sig;
    end if;
    select p.prosecdef,
           pg_catalog.pg_get_userbyid(p.proowner),
           pg_catalog.md5(pg_catalog.pg_get_functiondef(p.oid)),
    ...
    if v_owner is distinct from 'postgres'
       ...
       or pg_catalog.has_function_privilege('anon',v_oid,'EXECUTE')
       or pg_catalog.has_function_privilege('public',v_oid,'EXECUTE') then
      raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_WRITER_DRIFT signature=%',v_sig;
    end if;
  end loop;
  ...
  if v_oid is null
     ...
     or pg_catalog.has_function_privilege('anon',v_oid,'EXECUTE')
     or pg_catalog.has_function_privilege('public',v_oid,'EXECUTE') then
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_GERENCIAR_LISTA_DRIFT';
  end if;

  v_rows_before:=pg_catalog.current_setting('fechai.f1_02_b2_rollback_leads_rows')::bigint;
  select count(*) into v_rows_after from public.leads;
  if v_rows_after<>v_rows_before then
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_LEADS_DATA_MUTATION';
  end if;

  v_rows_before:=pg_catalog.current_setting('fechai.f1_02_b2_rollback_lotes_rows')::bigint;
  select count(*) into v_rows_after from public.lotes;
  if v_rows_after<>v_rows_before then
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_LOTES_DATA_MUTATION';
  end if;

  v_rows_before:=pg_catalog.current_setting('fechai.f1_02_b2_rollback_times_rows')::bigint;
  select count(*) into v_rows_after from public.times;
  if v_rows_after<>v_rows_before then
    raise exception 'F1_02_B2_ROLLBACK_POSTFLIGHT_TIMES_DATA_MUTATION';
  end if;
end;
$f1_02_b2_rollback_postflight$;

commit;

-- STOP CONDITION AFTER SUCCESSFUL ROLLBACK:
-- F1-02/B2 = OPEN / BLOCKING
-- SECURITY_GO = DENIED
