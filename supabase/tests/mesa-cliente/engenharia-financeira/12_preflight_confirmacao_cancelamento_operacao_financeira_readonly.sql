...
--   A 5C será a etapa que altera o status de uma operação financeira já registrada pela 5B.
--   Este arquivo NÃO cria dados, NÃO altera schema e NÃO chama RPC de escrita.
--
-- Regras:
--   - Somente SELECT.
--   - Seguro para SQL Editor do Supabase.
--   - Não depende de seed permanente.
--   - Não chama RPC 4B/5A/5B/5C.
--   - Não libera migration automaticamente; o resultset deve ser analisado.
--
-- Próximo passo após executar:
--   Enviar o resultset completo para fechar a migration/RPC real da 5C.
...
status_support as (
  select
    (
      exists(select 1 from constraints_inventory where definition ilike '%simulada%')
      or exists(select 1 from enum_inventory where labels::text ilike '%simulada%')
    ) as suporta_simulada,
    (
      exists(select 1 from constraints_inventory where definition ilike '%confirmada%')
      or exists(select 1 from enum_inventory where labels::text ilike '%confirmada%')
    ) as suporta_confirmada,
    (
      exists(select 1 from constraints_inventory where definition ilike '%cancelada%')
      or exists(select 1 from enum_inventory where labels::text ilike '%cancelada%')
    ) as suporta_cancelada,
    (
      exists(select 1 from constraints_inventory where definition ilike '%bloqueada%')
      or exists(select 1 from enum_inventory where labels::text ilike '%bloqueada%')
    ) as suporta_bloqueada
),
...
function_inventory as (
  ...
  select
    ...
    to_regprocedure('public.mesa_cliente_simular_impacto_agenda_persistida_admin(uuid,date,text,jsonb)') is not null,
    coalesce(has_function_privilege('anon', to_regprocedure('public.mesa_cliente_simular_impacto_agenda_persistida_admin(uuid,date,text,jsonb)'), 'EXECUTE'), false),
    coalesce(has_function_privilege('authenticated', to_regprocedure('public.mesa_cliente_simular_impacto_agenda_persistida_admin(uuid,date,text,jsonb)'), 'EXECUTE'), false)
  union all
  select
    '5B',
    'public.mesa_cliente_registrar_operacao_financeira_admin(uuid,uuid,text,uuid,date,date,numeric,jsonb)',
    to_regprocedure('public.mesa_cliente_registrar_operacao_financeira_admin(uuid,uuid,text,uuid,date,date,numeric,jsonb)') is not null,
    coalesce(has_function_privilege('anon', to_regprocedure('public.mesa_cliente_registrar_operacao_financeira_admin(uuid,uuid,text,uuid,date,date,numeric,jsonb)'), 'EXECUTE'), false),
    coalesce(has_function_privilege('authenticated', to_regprocedure('public.mesa_cliente_registrar_operacao_financeira_admin(uuid,uuid,text,uuid,date,date,numeric,jsonb)'), 'EXECUTE'), false)
  union all
  select
    '5C_CANDIDATA',
    'public.mesa_cliente_atualizar_status_operacao_financeira_admin(uuid,text,text,jsonb)',
    to_regprocedure('public.mesa_cliente_atualizar_status_operacao_financeira_admin(uuid,text,text,jsonb)') is not null,
    coalesce(has_function_privilege('anon', to_regprocedure('public.mesa_cliente_atualizar_status_operacao_financeira_admin(uuid,text,text,jsonb)'), 'EXECUTE'), false),
    coalesce(has_function_privilege('authenticated', to_regprocedure('public.mesa_cliente_atualizar_status_operacao_financeira_admin(uuid,text,text,jsonb)'), 'EXECUTE'), false)
),
...
select
  '02_colunas_operacoes_confirmacao_cancelamento'::text as bloco,
  case
    when bool_and(existe) filter (where grupo in ('core_5b', 'confirmacao_5c'))
      then case when bool_and(existe) filter (where grupo = 'cancelamento_5c') then 'PASS' else 'WARN' end
    else 'FAIL'
  end as status,
  jsonb_build_object(
    ...
    'interpretacao', case
      when not bool_and(existe) filter (where grupo = 'core_5b') then 'Faltam colunas base da 5B. Não criar 5C antes de corrigir.'
      when not bool_and(existe) filter (where grupo = 'confirmacao_5c') then 'Faltam colunas de confirmação. Migration 5C precisa corrigir antes da RPC.'
      when not bool_and(existe) filter (where grupo = 'cancelamento_5c') then 'Confirmação parece suportada, mas cancelamento exige colunas próprias ou decisão explícita por metadata.'
      else 'Colunas base, confirmação e cancelamento parecem presentes.'
    end,
...
select
  '03_status_operacao_suporte'::text as bloco,
  case
    when suporta_simulada and suporta_confirmada and suporta_cancelada then 'PASS'
    when suporta_simulada and suporta_confirmada then 'WARN'
    else 'FAIL'
  end as status,
  jsonb_build_object(
    ...
    ) order by conname) from constraints_inventory), '[]'::jsonb),
    'enums', coalesce((select jsonb_agg(jsonb_build_object(
      'enum_type', enum_type,
      'labels', labels
    ) order by enum_type) from enum_inventory), '[]'::jsonb),
    'interpretacao', case
      when suporta_simulada and suporta_confirmada and suporta_cancelada then 'Estados mínimos da 5C parecem suportados.'
      when suporta_simulada and suporta_confirmada then 'Cancelamento pode exigir ajuste de constraint/enum na migration 5C.'
      else 'Estados mínimos insuficientes para 5C. Revisar constraint/enum antes da RPC.'
    end
  ) as detalhe
from status_support
...
select
  '04_status_operacao_distribuicao_atual'::text as bloco,
  'INFO'::text as status,
  jsonb_build_object(
    ...
    'observacao', 'Leitura informativa. Não cria, não confirma e não cancela operações.'
  ) as detalhe
...
select
  '05_indices_operacoes_para_5c'::text as bloco,
  case
    when exists(select 1 from indexes_inventory where indexdef ilike '%agenda_id%' and indexdef ilike '%parcela%') then 'PASS'
    else 'WARN'
  end as status,
  jsonb_build_object(
...
select
  '06_triggers_operacoes_updated_at'::text as bloco,
  case
    when exists(select 1 from triggers_inventory where definition ilike '%updated_at%') then 'PASS'
    else 'WARN'
  end as status,
  jsonb_build_object(
    'tem_trigger_updated_at', exists(select 1 from triggers_inventory where definition ilike '%updated_at%'),
    'qtd_triggers', count(*),
    ...
    'observacao', 'Se houver trigger de updated_at, testes 12D/12E devem diferenciar mutação legítima da operação alvo versus mutação indevida de agenda/parcelas.'
  ) as detalhe
...
select
  '07_rls_policies_operacoes'::text as bloco,
  case
  ...
  jsonb_build_object(
    'rls', coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'rls_forced', rls_fored
    )) from (select schemaname, tablename, rls_enabled, rls_forced as rls_fored from rls_inventory) r), '[]'::jsonb),
...
select
  '08_grants_tabela_operacoes'::text as bloco,
  case
...
select
  '09_funcoes_dependencias_e_ausencia_5c'::text as bloco,
  case
    when not exists(select 1 from function_inventory where fase = '5B' and existe) then 'FAIL'
    when exists(select 1 from function_inventory where fase = '5C_CANDIDATA' and existe) then 'WARN'
    else 'PASS'
  end as status,
  jsonb_build_object(
    'rpc_5b_existe', exists(select 1 from function_inventory where fase = '5B' and existe),
    'rpc_5c_candidata_ja_existe', exists(select 1 from function_inventory where fase = '5C_CANDIDATA' and existe),
    ...
    ) order by fase, function_signature),
    'interpretacao', case
      when not exists(select 1 from function_inventory where fase = '5B' and existe) then 'RPC 5B não encontrada. Não criar 5C antes de corrigir base.'
      when exists(select 1 from function_inventory where fase = '5C_CANDIDATA' and existe) then 'RPC 5C candidata já existe. Verificar se não houve migration fora do protocolo.'
      else 'RPC 5B existe e RPC 5C ainda não existe, como esperado antes da migration 5C.'
    end
  ) as detalhe
...
select
  '10_funcoes_financeiras_security_definer'::text as bloco,
  case
    when exists(select 1 from function_security_inventory where proname = 'mesa_cliente_registrar_operacao_financeira_admin' and security_definer) then 'PASS'
    else 'WARN'
  end as status,
  jsonb_build_object(
    'funcoes', coalesce(jsonb_agg(jsonb_build_object(
      'schema', nspname,
      'proname', proname,
      'args', args,
...
select
  '11_readiness_para_migration_5c'::text as bloco,
  case
...
select
  ...
  jsonb_build_object(
    ...
    'proximos_arquivos_esperados_se_aprovado', jsonb_build_array(
      ...
      'supabase/tests/mesa-cliente/engenenharia-financeira/12b_validacao_cancelar_operacao_financeira_rollback.sql',
