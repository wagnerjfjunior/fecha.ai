...
-- 12A — Validação positiva de confirmação de operação financeira.
--
-- Pré-requisitos:
--   - main consolidada até 5B;
--   - migration 5C aplicada:
...
--   confirmar via RPC 5C e validar:
--     - operação simulada vira confirmada;
--     - confirmado=true, confirmado_por e confirmado_em preenchidos;
--     - cancelamento permanece nulo;
...
--
-- Observação:
--   Este teste não usa temp table. Usa set_config(...) transacional para evitar
--   fragilidade com SET LOCAL ROLE no SQL Editor do Supabase.

begin;

select set_config('app.mc12a.user_id', '', true);
select set_config('app.mc12a.simulacao_id', '', true);
select set_config('app.mc12a.empresa_id', '', true);
select set_config('app.mc12a.empreendimento_id', '', true);
select set_config('app.mc12a.politica_id', '', true);
select set_config('app.mc12a.agenda_id', '', true);
select set_config('app.mc12a.parcela_id', '', true);
select set_config('app.mc12a.payload_4b', 'null', true);
select set_config('app.mc12a.payload_5b', 'null', true);
select set_config('app.mc12a.payload_5c_confirmacao', 'null', true);
select set_config('app.mc12a.snapshot_before_5b', 'null', true);
select set_config('app.mc12a.snapshot_after_5b', 'null', true);
select set_config('app.mc12a.snapshot_after_5c', 'null', true);
...
simulacao as materialized (
  ...
  select
    ...
    corretor_id,
    empreendimento_id,
    'Teste rollback 12A confirmação operação financeira 5C',
    29500.50,
    10000.50,
    0,
    29500.50,
    jsonb_build_object('origem', 'teste_12a_5c_rollback', 'fixture_transacional', true),
    'Fixture transacional 12A. Deve sumir no ROLLBACK.'
  from candidato
...
politica as materialized (
  ...
  select
    ...
    true,
    'Fixture 12A para validação positiva da confirmação pela RPC 5C.'
  from simulacao
...
faixas as materialized (
  ...
  cross join (
    values
      (0.00::numeric, 2.00::numeric, 100.00::numeric, 'premio_cheio'::text, 'Faixa fixture 12A — prêmio cheio', 1),
      (2.01::numeric, 4.00::numeric, 70.00::numeric, 'premio_parcial'::text, 'Faixa fixture 12A — prêmio parcial', 2),
      (4.01::numeric, 6.00::numeric, 0.00::numeric, 'sem_premio'::text, 'Faixa fixture 12A — sem prêmio', 3)
...
setup as (
  select
    ...
    set_config('app.mc12a.user_id', c.user_id::text, true),
    set_config('app.mc12a.simulacao_id', s.id::text, true),
    set_config('app.mc12a.empresa_id', s.empresa_id::text, true),
    set_config('app.mc12a.empreendimento_id', s.empreendimento_id::text, true),
    set_config('app.mc12a.politica_id', p.id::text, true)
  from candidato c
  join simulacao s on true
  join politica p on true
)
select
  '00_setup_fixture_12a' as bloco,
  case when count(*) = 1 then 'PASS' else 'FAIL' end as status,
  jsonb_build_object(
    'simulacao_id', current_setting('app.mc12a.simulacao_id', true),
    'politica_id', current_setting('app.mc12a.politica_id', true),
...
with ctx as (
  select
    ...
    current_setting('app.mc12a.empresa_id', true)::uuid as empresa_id,
    current_setting('app.mc12a.empreendimento_id', true)::uuid as empreendimento_id
),
chamada_4b as materialized (
  select public.mesa_cliente_persistir_agenda_financeira_admin(
    ...
    jsonb_build_object('empresa_id', ctx.empresa_id, 'empreendimento_id', ctx.empreendimento_id, 'origem', 'teste_12a')
  ) as payload
  from ctx
)
select set_config('app.mc12a.payload_4b', coalesce((select payload::text from chamada_4b), 'null'), true);
...
setups as (
  select
    set_config('app.mc12a.agenda_id', (select id::text from agenda), true),
    set_config('app.mc12a.parcela_id', (select id::text from parcela), true),
    set_config('app.mc12a.snapshot_before_5b', (select payload::text from snapshot_before_5b), true)
)
select
  '00b_agenda_parcela_fixture_12a' as bloco,
  case
    when current_setting('app.mc12a.agenda_id', true) <> ''
     and current_setting('app.mc12a.parcela_id', true) <> ''
    then 'PASS' else 'FAIL'
  end as status,
  jsonb_build_object(
    'agenda_id', current_setting('app.mc12a.agenda_id', true),
    'parcela_id', current_setting('app.mc12a.parcela_id', true),
    'before_5b', current_setting('app.mc12a.snapshot_before_5b', true)::jsonb
  ) as detalhe
from setups;
...
with chamada_5b as materialized (
  select public.mesa_cliente_registrar_operacao_financeira_admin(
    current_setting('app.mc12a.simulacao_id', true)::uuid,
    current_setting('app.mc12a.agenda_id', true)::uuid,
    'antecipacao',
    current_setting('app.mc12a.parcela_id', true)::uuid,
    date '2099-05-31',
    null,
    5000.00,
    jsonb_build_object('origem_teste', '12a', 'observacao', 'operacao simulada para confirmacao 5C')
  ) as payload
)
select set_config('app.mc12a.payload_5b', coalesce((select payload::text from chamada_5b), 'null'), true);
...
select set_config('app.mc12a.snapshot_after_5b', coalesce((select payload::text from snapshot_after_5b), 'null'), true);

set local role authenticated;

with chamada_5c as materialized (
  select public.mesa_cliente_atualizar_status_operacao_financeira_admin(
    (current_setting('app.mc12a.payload_5b', true)::jsonb->'operacao'->>'id')::uuid,
    'confirmar',
    null,
    jsonb_build_object('origem_teste', '12a', 'observacao', 'confirmacao positiva 5C')
  ) as payload
)
select set_config('app.mc12a.payload_5c_confirmacao', coalesce((select payload::text from chamada_5c), 'null'), true);
...
select set_config('app.mc12a.snapshot_after_5c', coalesce((select payload::text from snapshot_after_5c), 'null'), true);

with dados as (
  select
    current_setting('app.mc12a.snapshot_before_5b', true)::jsonb as b,
    current_setting('app.mc12a.snapshot_after_5b', true)::jsonb as s5b,
    current_setting('app.mc12a.snapshot_after_5c', true)::jsonb as s5c,
    current_setting('app.mc12a.payload_5b', true)::jsonb as p5b,
    current_setting('app.mc12a.payload_5c_confirmacao', true)::jsonb as p5c,
    current_setting('app.mc12a.user_id', true) as user_id
)
select
  bloco,
  status,
...
from (
  ...
  select
    ...
    jsonb_build_object(
      'payload_5c_flags', jsonb_build_object(
        'fase', p5c->>'fase',
        'acao', p5c->>'acao',
        'cliente_safe', p5c->>'cliente_safe',
        'persistencia', p5c->>'persistencia',
        'dml_financeiro', p5c->>'dml_financeiro',
        'escopo_dml', p5c->>'escopo_dml',
        'altera_agenda', p5c->>'altera_agenda',
        'altera_parcelas', p5c->>'altera_parcelas',
        'recalcula_operacao', p5c->>'recalcula_operacao',
        'idempotente', p5c->>'idempotente'
      )
    )
  from dados
  ...
  select
    '03_operacao_confirmada_campos_auditoria',
    case
      when p5c->'operacao'->>'id' = p5b->'operacao'->>'id'
       and p5c->'operacao'->>'status_operacao_anterior' = 'simulada'
       and p5c->'operacao'->>'status_operacao' = 'confirmada'
       and p5c->'operacao'->>'confirmado' = 'true'
       and p5c->'operacao'->>'confirmado_por' = user_id
       and p5c->'operacao'->>'confirmado_em' is not null
       and p5c->'operacao'->>'cancelado_por' is null
       and p5c->'operacao'->>'cancelado_em' is null
       and p5c->'operacao'->>'motivo_cancelamento' is null
       and p5c->'operacao'->>'visivel_cliente' = 'false'
       and p5c->'operacao'->>'checksum_operacao' = p5b->'operacao'->>'checksum_operacao'
    then 'PASS' else 'FAIL' end,
    jsonb_build_object(
      'operacao_5b', p5b->'operacao',
      'operacao_5c', p5c->'operacao',
      'user_id_esperado', user_id
    )
  from dados
  ...
  select
    '04_estado_banco_confirmado_sem_exposicao_cliente',
    case
      when (s5c->>'operacoes')::integer = 1
       and (s5c->>'operacoes_confirmadas')::integer = 1
       and (s5c->>'operacoes_canceladas')::integer = 0
       and (s5c->>'operacoes_visiveis_cliente')::integer = 0
       and s5c->'operacoes_lista'->0->>'id' = p5b->'operacao'->>'id'
       and s5c->'operacoes_lista'->0->>'status_operacao' = 'confirmada'
       and s5c->'operacoes_lista'->0->>'confirmado' = 'true'
       and s5c->'operacoes_lista'->0->>'confirmado_por' = user_id
       and s5c->'operacoes_lista'->0->>'confirmado_em' is not null
       and s5c->'operacoes_lista'->0->>'cancelado_por' is null
       and s5c->'operacoes_lista'->0->>'cancelado_em' is null
       and s5c->'operacoes_lista'->0->>'motivo_cancelamento' is null
       and s5c->'operacoes_lista'->0->>'visivel_cliente' = 'false'
       and s5c->'operacoes_lista'->0->'metadata_5c'->>'acao' = 'confirmar'
    then 'PASS' else 'FAIL' end,
    jsonb_build_object(
      'snapshot_after_5c', s5c
    )
  from dados
  ...
  select
    ...
    then 'PASS' else 'FAIL' end,
    jsonb_build_object(
      'updated_at_retorno', p5c->'operacao'->>'updated_at',
      'updated_at_banco', s5c->'operacoes_lista'->0->>'updated_at'
    )
  from dados
  ...
  select
    '99_rollback_notice',
    'INFO',
    jsonb_build_object('mensagem', 'Teste 12A encerra com ROLLBACK. Nada deve permanecer no banco.')
) r
order by bloco;
