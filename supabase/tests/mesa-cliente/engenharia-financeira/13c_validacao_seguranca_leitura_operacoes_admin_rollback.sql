...
--
-- Correção estrutural do harness 13C:
--   Este teste NÃO usa tabela temporária de resultados.
--   O acumulador de evidências fica em app.mc13c.results como JSONB transacional.
--   Motivo: eliminar definitivamente o erro 42P01 relation tmp_13c_resultados does not exist
--   sem remover nenhum bloco de validação de segurança.
--
-- Segurança do teste:
--   - cria fixture transacional mínima via 4B/5B;
--   - executa somente negativos contra 5D;
--   - valida grants esperados;
--   - compara hashes antes/depois;
--   - encerra com ROLLBACK.

begin;

select set_config('app.mc13c.results', '[]', true);
...
create or replace function pg_temp.mc13c_add_result(
  p_bloco text,
  p_status text,
...
declare
  v_atual jsonb;
begin
  v_atual := coalesce(nullif(current_setting('app.mc13c.results', true), '')::jsonb, '[]'::jsonb);
...
create or replace function pg_temp.mc13c_expect_error(
  p_bloco text,
...
begin
  ...
  perform pg_temp.mc13c_add_result(
    p_bloco,
    'FAIL',
...
select pg_temp.mc13c_add_result(
...
select pg_temp.mc13c_expect_error(
  '03a_listar_sem_auth_bloqueado',
  '28000',
  format($sql$select public.mesa_cliente_listar_operacoes_financeiras_admin(%L::uuid, null, '{}'::jsonb)$sql$, current_setting('app.mc13c.simulacao_id', true)),
  'chamada sem auth.uid() foi aceita'
);

select pg_temp.mc13c_expect_error(
  '03b_obter_sem_auth_bloqueado',
  '28000',
  format($sql$select public.mesa_cliente_obter_operacao_financeira_admin(%L::uuid, '{}'::jsonb)$sql$, current_setting('app.mc13c.operacao_id', true)),
  'chamada sem auth.uid() foi aceita'
);
...
select pg_temp.mc13c_expect_error(
  '04a_listar_simulacao_inexistente_bloqueada',
  'P0002',
  $sql$select public.mesa_cliente_listar_operacoes_financeiras_admin('00000000-0000-0000-0000-000000000001'::uuid, null, '{}'::jsonb)$sql$,
  'simulação inexistente foi aceita'
);

select pg_temp.mc13c_expect_error(
  '04b_obter_operacao_inexistente_bloqueada',
  'P0002',
  $sql$select public.mesa_cliente_obter_operacao_financeira_admin('00000000-0000-0000-0000-000000000001'::uuid, '{}'::jsonb)$sql$,
  'operação inexistente foi aceita'
);

select pg_temp.mc13c_expect_error(
  '05a_listar_filtros_nao_objeto_bloqueado',
  '22023',
  format($sql$select public.mesa_cliente_listar_operacoes_financeiras_admin(%L::uuid, null, '[]'::jsonb)$sql$, current_setting('app.mc13c.simulacao_id', true)),
  'p_filtros array foi aceito'
);

select pg_temp.mc13c_expect_error(
  '05b_obter_parametros_nao_objeto_bloqueado',
  '22023',
  format($sql$select public.mesa_cliente_obter_operacao_financeira_admin(%L::uuid, '[]'::jsonb)$sql$, current_setting('app.mc13c.operacao_id', true)),
  'p_parametros array foi aceito'
);

select pg_temp.mc13c_expect_error(
  '06a_listar_payload_soberano_bloqueado',
  '42501',
  format(
    $sql$select public.mesa_cliente_listar_operacoes_financeiras_admin(%L::uuid, null, jsonb_build_object('empresa_id', %L))$sql$,
    current_setting('app.mc13c.simulacao_id', true),
    current_setting('app.mc13c.empresa_id', true)
  ),
  'empresa_id em p_filtros foi aceito'
);

select pg_temp.mc13c_expect_error(
  '06b_obter_payload_soberano_bloqueado',
  '42501',
  format(
    $sql$select public.mesa_cliente_obter_operacao_financeira_admin(%L::uuid, jsonb_build_object('simulacao_id', %L))$sql$,
    current_setting('app.mc13c.operacao_id', true),
    current_setting('app.mc13c.simulacao_id', true)
  ),
  'simulacao_id em p_parametros foi aceito'
);

select pg_temp.mc13c_expect_error(
  '07a_listar_status_invalido_bloqueado',
  '22023',
  format($sql$select public.mesa_cliente_listar_operacoes_financeiras_admin(%L::uuid, null, jsonb_build_object('status_operacao', 'hack'))$sql$, current_setting('app.mc13c.simulacao_id', true)),
  'status inválido foi aceito'
);

select pg_temp.mc13c_expect_error(
  '07b_listar_limit_invalido_bloqueado',
  '22023',
  format($sql$select public.mesa_cliente_listar_operacoes_financeiras_admin(%L::uuid, null, jsonb_build_object('limit', 201))$sql$, current_setting('app.mc13c.simulacao_id', true)),
  'limit inválido foi aceito'
);

select pg_temp.mc13c_expect_error(
  '07c_listar_agenda_incompativel_bloqueada',
  'P0002',
  format($sql$select public.mesa_cliente_listar_operacoes_financeiras_admin(%L::uuid, '00000000-0000-0000-0000-000000000001'::uuid, '{}'::jsonb)$sql$, current_setting('app.mc13c.simulacao_id', true)),
  'agenda incompatível/inexistente foi aceita'
);
...
begin
  ...
  begin
    ...
    perform pg_temp.mc13c_add_result('08a_listar_cross_tenant_bloqueado', 'FAIL', jsonb_build_object('erro', 'usuário de outro tenant listou simulação fixture'));
  exception when others then
    get stacked diagnostics v_msg = message_text;
    v_state := sqlstate;
    perform pg_temp.mc13c_add_result('08a_listar_cross_tenant_bloqueado', case when v_state = '42501' then 'PASS' else 'FAIL' end, jsonb_build_object('sqlstate', v_state, 'message', v_msg));
  end;

  begin
    perform public.mesa_cliente_obter_operacao_financeira_admin(current_setting('app.mc13c.operacao_id', true)::uuid, '{}'::jsonb);
    perform pg_temp.mc13c_add_result('08b_obter_cross_tenant_bloqueado', 'FAIL', jsonb_build_object('erro', 'usuário de outro tenant obteve operação fixture'));
  exception when others then
    get stacked diagnostics v_msg = message_text;
    v_state := sqlstate;
    perform pg_temp.mc13c_add_result('08b_obter_cross_tenant_bloqueado', case when v_state = '42501' then 'PASS' else 'FAIL' end, jsonb_build_object('sqlstate', v_state, 'message', v_msg));
  end;
end $$;
...
select pg_temp.mc13c_add_result(
  ...
  case
    when current_setting('app.mc13c.snapshot_before', true)::jsonb->>'agenda_full_hash' = current_setting('app.mc13c.snapshot_after', true)::jsonb->>'agenda_full_hash'
     and current_setting('app.mc13c.snapshot_before', true)::jsonb->>'parcelas_full_hash' = current_setting('app.mc13c.snapshot_after', true)::jsonb->>'parcelas_full_hash'
     and current_setting('app.mc13c.snapshot_before', true)::jsonb->>'operacoes_full_hash' = current_setting('app.mc13c.snapshot_after', true)::jsonb->>'operacoes_full_hash'
    then 'PASS' else 'FAIL'
  end,
  jsonb_build_object(
    'before', current_setting('app.mc13c.snapshot_before', true)::jsonb,
    'after', current_setting('app.mc13c.snapshot_after', true)::jsonb,
    'hash_agenda_igual', current_setting('app.mc13c.snapshot_before', true)::jsonb->>'agenda_full_hash' = current_setting('app.mc13c.snapshot_after', true)::jsonb->>'agenda_full_hash',
    'hash_parcelas_igual', current_setting('app.mc13c.snapshot_before', true)::jsonb->>'parcelas_full_hash' = current_setting('app.mc13c.snapshot_after', true)::jsonb->>'parcelas_full_hash',
    'hash_operacoes_igual', current_setting('app.mc13c.snapshot_before', true)::jsonb->>'operacoes_full_hash' = current_setting('app.mc13c.snapshot_after', true)::jsonb->>'operacoes_full_hash'
  )
);
...
with resultados as (
  select
    ord::integer as ordem,
    item->>'bloco' as bloco,
    item->>'status' as status,
    coalesce(item->'detalhe', '{}'::jsonb) as detalhe
  from jsonb_array_elements(current_setting('app.mc13c.results', true)::jsonb) with ordinality as r(item, ord)
)
select jsonb_pretty(
  jsonb_agg(
    jsonb_build_object(
...
) as resultado_13c_validacao_seguranca_leitura_operacoes_admin;
