...
-- RPC administrativa agenda-first para simular impacto financeiro sobre agenda persistida.
--
-- Contrato:
...
--   - Não altera agenda.
--   - Não altera parcelas.
--   - Não usa payload cliente-safe como fonte soberana.
--   - Não aceita empresa_id do frontend como autoridade.
--   - Usa agenda ativa persistida + política financeira vigente como base.
--   - Retorna visão administrativa, cliente_safe=false.
...
begin
  ...
  if v_qtd_confirmadas > 0 then
    raise exception using
      errcode = '55000',
      message = 'Simulação bloqueada: já existe operação financeira confirmada para esta agenda/simulação.';
  end if;
...
is 'FECH.AI MesaCliente Fase 5A.1: simulação administrativa agenda-first de impacto financeiro sobre agenda persistida, sem DML financeiro e sem cliente_safe.';
