-- FECH.AI — F1-02 / B2
-- Restrict authenticated direct CRM writes while preserving the reviewed
-- SECURITY DEFINER compatibility-writer boundary.
--
-- VERSIONED ARTIFACT ONLY.
-- This file is NOT authorization to apply this migration to Supabase.
...
begin
  if pg_catalog.to_regrole('authenticated') is null
    ...
    raise exception 'F1_02_B2_PREFLIGHT_REQUIRED_ROLE_MISSING';
  end if;
  ...
  if pg_catalog.to_regclass('public.leads') is null
    ...
    raise exception 'F1_02_B2_PREFLIGHT_REQUIRED_TABLE_MISSING';
  end if;

  -- Owners and RLS/FORCE RLS invariants.
  for v_sig in
  ...
  loop
    ...
    if v_owner is distinct from 'postgres'
      ...
      raise exception
        'F1_02_B2_PREFLIGHT_TABLE_INVARIANT_DRIFT table=% owner=% rls=% force_rls=%',
        v_sig, v_owner, v_rls, v_force_rls;
    end if;
  end loop;

  -- Exact reviewed ACL baselines. These are preflight drift guards only.
  select pg_catalog.md5(coalesce(c.relacl::text,''))
    into v_hash
  from pg_catalog.pg_class c
  where c.oid = 'public.leads'::pg_catalog.regclass;
  if v_hash is distinct from '08a95c631129535b0e07e9be92672b71' then
    raise exception 'F1_02_B2_PREFLIGHT_LEADS_RELACL_DRIFT hash=%', v_hash;
  end if;
  ...
  if v_hash is distinct from 'afa3a93809a23f744356971cbc461855' then
    raise exception 'F1_02_B2_PREFLIGHT_LOTES_RELACL_DRIFT hash=%', v_hash;
  end if;
  ...
  if v_hash is distinct from 'b737d3eae6acb29657f866f2f8993734' then
    raise exception 'F1_02_B2_PREFLIGHT_TIMES_RELACL_DRIFT hash=%', v_hash;
  end if;

  -- Reviewed authenticated direct-grant baseline.
  if not pg_catalog.has_table_privilege('authenticated','public.leads','SELECT')
     or not pg_catalog.has_table_privilege('authenticated','public.leads','INSERT')
     or not pg_catalog.has_table_privilege('authenticated','public.leads','UPDATE')
     or pg_catalog.has_table_privilege('authenticated','public.leads','DELETE')
     or not pg_catalog.has_any_column_privilege('authenticated','public.leads','INSERT')
     or not pg_catalog.has_any_column_privilege('authenticated','public.leads','UPDATE') then
    raise exception 'F1_02_B2_PREFLIGHT_LEADS_AUTHENTICATED_GRANT_DRIFT';
  end if;
  ...
  if not pg_catalog.has_table_privilege('authenticated','public.lotes','SELECT')
     ...
     or not pg_catalog.has_table_privilege('authenticated','public.lotes','UPDATE')
     ...
     or not pg_catalog.has_any_column_privilege('authenticated','public.lotes','UPDATE') then
    raise exception 'F1_02_B2_PREFLIGHT_LOTES_AUTHENTICATED_GRANT_DRIFT';
  end if;
  ...
  if not pg_catalog.has_table_privilege('authenticated','public.times','SELECT')
    ...
    raise exception 'F1_02_B2_PREFLIGHT_TIMES_AUTHENTICATED_GRANT_DRIFT';
  end if;

  -- No target-column ACLs are allowed.
  select count(*)
    into v_count
  ...
  if v_count <> 0 then
    raise exception 'F1_02_B2_PREFLIGHT_COLUMN_ACL_PRESENT count=%', v_count;
  end if;

  -- anon remains unable to write directly.
  if pg_catalog.has_table_privilege('anon','public.leads','INSERT')
    ...
    raise exception 'F1_02_B2_PREFLIGHT_ANON_WRITE_PRESENT';
  end if;

  -- Privileged-role DML rights are collateral invariants.
  for v_sig in
    select unnest(array['public.leads','public.lotes','public.times'])
  loop
    if not pg_catalog.has_table_privilege('service_role',v_sig,'SELECT')
      ...
      raise exception 'F1_02_B2_PREFLIGHT_PRIVILEGED_ROLE_DRIFT table=%', v_sig;
    end if;
  end loop;

  -- Policy fingerprints: intended to remain unchanged.
  select count(*),
  ...
  if v_count <> 3 or v_hash is distinct from '360bc33f4dd17937172e98da5203a220' then
    raise exception 'F1_02_B2_PREFLIGHT_LEADS_POLICY_DRIFT count=% hash=%', v_count, v_hash;
  end if;
  ...
  if v_count <> 2 or v_hash is distinct from 'ac27dc669f8136de8004e03ff3ba2276' then
    raise exception 'F1_02_B2_PREFLIGHT_LOTES_POLICY_DRIFT count=% hash=%', v_count, v_hash;
  end if;
  ...
  if v_count <> 3 or v_hash is distinct from '20cd727acb7a0b9475e0689949e26f84' then
    raise exception 'F1_02_B2_PREFLIGHT_TIMES_POLICY_DRIFT count=% hash=%', v_count, v_hash;
  end if;
  ...
  if v_count <> 2 or v_hash is distinct from 'f1461871ed8742cda6091953c42159e7' then
    raise exception 'F1_02_B2_PREFLIGHT_LEADS_TRIGGER_DRIFT count=% hash=%', v_count, v_hash;
  end if;
  ...
  if v_count <> 0 or v_hash is distinct from 'd41d8cd98f00b204e9800998ecf8427e' then
    raise exception 'F1_02_B2_PREFLIGHT_LOTES_TRIGGER_DRIFT count=% hash=%', v_count, v_hash;
  end if;
  ...
  if v_count <> 2 or v_hash is distinct from '3b96724deabeb92d0eedd53e617d5625' then
    raise exception 'F1_02_B2_PREFLIGHT_TIMES_TRIGGER_DRIFT count=% hash=%', v_count, v_hash;
  end if;
  ...
  if v_count <> 13 or v_hash is distinct from '3fc58a96da3b1235dc5af1cbb5f49a57' then
    raise exception 'F1_02_B2_PREFLIGHT_LEADS_FK_DRIFT count=% hash=%', v_count, v_hash;
  end if;
  ...
  if v_count <> 4 or v_hash is distinct from '69573308b1550d317555724642bb6365' then
    raise exception 'F1_02_B2_PREFLIGHT_LOTES_FK_DRIFT count=% hash=%', v_count, v_hash;
  end if;
  ...
  if v_count <> 2 or v_hash is distinct from 'ff8a1effa1eb4df4f52361ab3dc3d651' then
    raise exception 'F1_02_B2_PREFLIGHT_TIMES_FK_DRIFT count=% hash=%', v_count, v_hash;
  end if;

  -- Required SECURITY DEFINER compatibility writers.
  ...
  loop
    ...
    if v_oid is null then
      raise exception 'F1_02_B2_PREFLIGHT_WRITER_MISSING signature=%', v_sig;
    end if;
    ...
    if v_owner is distinct from 'postgres'
      ...
      raise exception
        'F1_02_B2_PREFLIGHT_WRITER_DRIFT signature=% owner=% secdef=% def=% acl=%',
        v_sig, v_owner, v_secdef, v_actual_def, v_actual_acl;
    end if;
  end loop;

  -- gerenciar_lista must remain unavailable and unchanged.
  v_oid := pg_catalog.to_regprocedure('public.gerenciar_lista(uuid,text,text)');
  if v_oid is null then
    raise exception 'F1_02_B2_PREFLIGHT_GERENCIAR_LISTA_MISSING';
  end if;
  ...
  if v_owner is distinct from 'postgres'
     ...
     or pg_catalog.has_function_privilege('anon',v_oid,'EXECUTE')
     or pg_catalog.has_function_privilege('public',v_oid,'EXECUTE') then
    raise exception 'F1_02_B2_PREFLIGHT_GERENCIAR_LISTA_DRIFT';
  end if;

  -- Snapshot business-row counts inside this transaction; the migration itself
  -- must not modify business rows.
  select count(*) into v_rows_leads from public.leads;
  select count(*) into v_rows_lotes from public.lotes;
  select count(*) into v_rows_times from public.times;

  perform pg_catalog.set_config('fechai.f1_02_b2_leads_rows',v_rows_leads::text,true);
  perform pg_catalog.set_config('fechai.f1_02_b2_lotes_rows',v_rows_lotes::text,true);
  perform pg_catalog.set_config('fechai.f1_02_b2_times_rows',v_rows_times::text,true);
end;
$f1_02_b2_preflight$;

-- The complete approved B2 security delta.
revoke insert, update on table public.leads from authenticated;
revoke update on table public.lotes from authenticated;

do $f1_02_b2_postflight$
declare
  v_count integer;
  v_hash text;
  v_owner text;
...
begin
  ...
  if not pg_catalog.has_table_privilege('authenticated','public.leads','SELECT')
    ...
    raise exception 'F1_02_B2_POSTFLIGHT_LEADS_GRANT_FAILURE';
  end if;
  ...
  if not pg_catalog.has_table_privilege('authenticated','public.lotes','SELECT')
    ...
    raise exception 'F1_02_B2_POSTFLIGHT_LOTES_GRANT_FAILURE';
  end if;
  ...
  if not pg_catalog.has_table_privilege('authenticated','public.times','SELECT')
     ...
     or pg_catalog.has_any_column_privilege('authenticated','public.times','INSERT')
     or pg_catalog.has_any_column_privilege('authenticated','public.times','UPDATE') then
    raise exception 'F1_02_B2_POSTFLIGHT_TIMES_GRANT_DRIFT';
  end if;

  -- times ACL is an exact non-change invariant.
  select pg_catalog.md5(coalesce(c.relacl::text,''))
    into v_hash
  from pg_catalog.pg_class c
  where c.oid='public.times'::pg_catalog.regclass;
  if v_hash is distinct from 'b737d3eae6acb29657f866f2f8993734' then
    raise exception 'F1_02_B2_POSTFLIGHT_TIMES_RELACL_DRIFT hash=%',v_hash;
  end if;

  -- Owners and RLS/FORCE RLS remain unchanged.
  for v_sig in
    select unnest(array['public.leads','public.lotes','public.times'])
  loop
    select pg_catalog.pg_get_userbyid(c.relowner),
    ...
    if v_owner is distinct from 'postgres'
       or v_rls is distinct from true
       or v_force_rls is distinct from true then
      raise exception 'F1_02_B2_POSTFLIGHT_TABLE_INVARIANT_DRIFT table=%',v_sig;
    end if;
  end loop;

  -- No explicit target-column ACL was introduced.
  select count(*)
    into v_count
  from pg_catalog.pg_attribute a
  cross join lateral pg_catalog.aclexplode(a.attacl) x
  where a.attrelid in (
    ...
    and a.attacl is not null;
  if v_count <> 0 then
    raise exception 'F1_02_B2_POSTFLIGHT_COLUMN_ACL_PRESENT count=%',v_count;
  end if;
  ...
  if pg_catalog.has_table_privilege('anon','public.leads','INSERT')
     ...
     or pg_catalog.has_table_privilege('anon','public.times','UPDATE')
     or pg_catalog.has_table_privilege('anon','public.times','DELETE') then
    raise exception 'F1_02_B2_POSTFLIGHT_ANON_WRITE_PRESENT';
  end if;
  ...
  loop
    if not pg_catalog.has_table_privilege('service_role',v_sig,'SELECT')
       ...
       or not pg_catalog.has_table_privilege('postgres',v_sig,'UPDATE')
       or not pg_catalog.has_table_privilege('postgres',v_sig,'DELETE') then
      raise exception 'F1_02_B2_POSTFLIGHT_PRIVILEGED_ROLE_DRIFT table=%',v_sig;
    end if;
  end loop;

  -- Policy fingerprints remain exactly unchanged.
  select count(*),
         pg_catalog.md5(coalesce(pg_catalog.string_agg(
           p.policyname || '|' || p.cmd || '|' || p.permissive::text || '|' ||
           coalesce(pg_catalog.array_to_string(p.roles,','),'') || '|' ||
  ...
  where p.schemaname='public' and p.tablename='leads';
  if v_count <> 3 or v_hash is distinct from '360bc33f4dd17937172e98da5203a220' then
    raise exception 'F1_02_B2_POSTFLIGHT_LEADS_POLICY_DRIFT';
  end if;
  ...
  where p.schemaname='public' and p.tablename='lotes';
  if v_count <> 2 or v_hash is distinct from 'ac27dc669f8136de8004e03ff3ba2276' then
    raise exception 'F1_02_B2_POSTFLIGHT_LOTES_POLICY_DRIFT';
  end if;
  ...
  where p.schemaname='public' and p.tablename='times';
  if v_count <> 3 or v_hash is distinct from '20cd727acb7a0b9475e0689949e26f84' then
    raise exception 'F1_02_B2_POSTFLIGHT_TIMES_POLICY_DRIFT';
  end if;

  -- Trigger fingerprints remain unchanged.
  select count(*),
         pg_catalog.md5(coalesce(pg_catalog.string_agg(
           t.tgname || '|' || t.tgenabled::text || '|' ||
           t.tgfoid::pg_catalog.regprocedure::text,
  ...
  where t.tgrelid='public.leads'::pg_catalog.regclass and not t.tgisinternal;
  if v_count <> 2 or v_hash is distinct from 'f1461871ed8742cda6091953c42159e7' then
    raise exception 'F1_02_B2_POSTFLIGHT_LEADS_TRIGGER_DRIFT';
  end if;
  ...
  where t.tgrelid='public.lotes'::pg_catalog.regclass and not t.tgisinternal;
  if v_count <> 0 or v_hash is distinct from 'd41d8cd98f00b204e9800998ecf8427e' then
    raise exception 'F1_02_B2_POSTFLIGHT_LOTES_TRIGGER_DRIFT';
  end if;
  ...
  where t.tgrelid='public.times'::pg_catalog.regclass and not t.tgisinternal;
  if v_count <> 2 or v_hash is distinct from '3b96724deabeb92d0eedd53e617d5625' then
    raise exception 'F1_02_B2_POSTFLIGHT_TIMES_TRIGGER_DRIFT';
  end if;

  -- FK fingerprints remain unchanged.
  select count(*),
         pg_catalog.md5(coalesce(pg_catalog.string_agg(
           co.conname || '|' || co.convalidated::text || '|' ||
           pg_catalog.pg_get_constraintdef(co.oid,true),
  ...
  where co.conrelid='public.leads'::pg_catalog.regclass and co.contype='f';
  if v_count <> 13 or v_hash is distinct from '3fc58a96da3b1235dc5af1cbb5f49a57' then
    raise exception 'F1_02_B2_POSTFLIGHT_LEADS_FK_DRIFT';
  end if;
  ...
  where co.conrelid='public.lotes'::pg_catalog.regclass and co.contype='f';
  if v_count <> 4 or v_hash is distinct from '69573308b1550d317555724642bb6365' then
    raise exception 'F1_02_B2_POSTFLIGHT_LOTES_FK_DRIFT';
  end if;
  ...
  where co.conrelid='public.times'::pg_catalog.regclass and co.contype='f';
  if v_count <> 2 or v_hash is distinct from 'ff8a1effa1eb4df4f52361ab3dc3d651' then
    raise exception 'F1_02_B2_POSTFLIGHT_TIMES_FK_DRIFT';
  end if;

  -- Required writers remain byte-contract compatible.
  for v_sig, v_expected_def, v_expected_acl in
    select *
    from (values
      ('public.atualizar_feedback(uuid,text,text)','57ba2f5d9cbc65cbbd9ed213265b184c','06c8bd810f2d9a52a993cd903c13793a'),
  ...
  loop
    v_oid := pg_catalog.to_regprocedure(v_sig);
    if v_oid is null then
      raise exception 'F1_02_B2_POSTFLIGHT_WRITER_MISSING signature=%',v_sig;
    end if;
    ...
    if v_owner is distinct from 'postgres'
       ...
       or pg_catalog.has_function_privilege('anon',v_oid,'EXECUTE')
       or pg_catalog.has_function_privilege('public',v_oid,'EXECUTE') then
      raise exception 'F1_02_B2_POSTFLIGHT_WRITER_DRIFT signature=%',v_sig;
    end if;
  end loop;

  v_oid := pg_catalog.to_regprocedure('public.gerenciar_lista(uuid,text,text)');
  if v_oid is null then
    raise exception 'F1_02_B2_POSTFLIGHT_GERENCIAR_LISTA_MISSING';
  end if;
  ...
  if v_owner is distinct from 'postgres'
     ...
     or pg_catalog.has_function_privilege('anon',v_oid,'EXECUTE')
     or pg_catalog.has_function_privilege('public',v_oid,'EXECUTE') then
    raise exception 'F1_02_B2_POSTFLIGHT_GERENCIAR_LISTA_DRIFT';
  end if;

  -- This grant-only migration must not mutate business rows.
  v_rows_before := pg_catalog.current_setting('fechai.f1_02_b2_leads_rows')::bigint;
  select count(*) into v_rows_after from public.leads;
  if v_rows_after <> v_rows_before then
    raise exception 'F1_02_B2_POSTFLIGHT_LEADS_DATA_MUTATION before=% after=%',v_rows_before,v_rows_after;
  end if;

  v_rows_before := pg_catalog.current_setting('fechai.f1_02_b2_lotes_rows')::bigint;
  select count(*) into v_rows_after from public.lotes;
  if v_rows_after <> v_rows_before then
    raise exception 'F1_02_B2_POSTFLIGHT_LOTES_DATA_MUTATION before=% after=%',v_rows_before,v_rows_after;
  end if;

  v_rows_before := pg_catalog.current_setting('fechai.f1_02_b2_times_rows')::bigint;
  select count(*) into v_rows_after from public.times;
  if v_rows_after <> v_rows_before then
    raise exception 'F1_02_B2_POSTFLIGHT_TIMES_DATA_MUTATION before=% after=%',v_rows_before,v_rows_after;
  end if;
end;
$f1_02_b2_postflight$;
...
-- ESTABLISHES ONLY IF APPLIED AND POSTFLIGHT PASSES:
--   authenticated direct leads INSERT/UPDATE removed
--   authenticated direct lotes UPDATE removed
-- DOES NOT ESTABLISH:
