-- FECH.AI / MesaCliente — Fase 20A.1
-- Hardening da RPC de fluxo histórico.
-- Regra comercial: outro tenant/empresa sempre bloqueado.
-- Dentro do mesmo tenant, somente corretor dono ou gestor do mesmo time acessa proposta/fluxo de cliente.
-- Admin local, admin global e root não acessam por esta RPC comum.
...
begin
  ...
  if v_user_role in ('admin_global', 'root')
...
is 'Fase 20A.1: leitura tenant-safe/read-only de fluxo histórico. Acesso comercial restrito a corretor dono ou gestor do mesmo time. Admin/root não acessam por esta RPC comum.';
