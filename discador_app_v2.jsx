...
// ================================================================
// DISCADOR v2.0 — Sistema de Gestão de Leads Imobiliários
// ================================================================
...
function Stars({ value, onChange }) {
  return (<div className="flex gap-1">{[1, 2, 3, 4, 5].map(n => (<button key={n} className={`text-2xl ${n <= value ? "text-amber-400" : "text-gray-300"}`} onClick={() => onChange?.(n)}>{n <= value ? "★" : "☆"}</button>))}</div>);
}
...
function ConfigScreen({ onSave }) {
  ...
  return (<div className="min-h-screen bg-gray-50 flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-md"><h1 className="text-2xl font-bold text-gray-900 mb-1">Discador</h1><p className="text-gray-500 text-sm mb-6">Configure a conexão com o Supabase</p><label className="block text-sm font-medium text-gray-700 mb-1">URL do Supabase</label><input className="w-full border border-gray-300 rounded-lg px-3 py-2 mb-4 text-sm" placeholder="https://xxx.supabase.co" value={url} onChange={e => setUrl(e.target.value)} /><label className="block text-sm font-medium text-gray-700 mb-1">Anon Key</label><input className="w-full border border-gray-300 rounded-lg px-3 py-2 mb-6 text-sm font-mono" placeholder="eyJhbG..." value={key} onChange={e => setKey(e.target.value)} /><button className="w-full bg-blue-600 text-white rounded-lg py-3 font-medium" onClick={() => url && key && onSave({ url: url.replace(/\/+$/, ""), key })}>Conectar</button></div></div>);
}
...
function DiscadorTab({ sb, token, corretor }) {
  ...
  const [showRate, setShowRate] = useState(false); const [rateNote, setRateNote] = useState(0);

  const loadNext = useCallback(async () => {
    setLd(true); setLoteDone(false);
    try { const r = await sb.rpc("proximo_lead", {}, token); setLead(r.lead ? (typeof r.lead === "string" ? JSON.parse(r.lead) : r.lead) : null); setProg(r.progresso || null); setMsg(r.message || ""); } catch (e) { setMsg(e.message); }
    setLd(false);
  }, [sb, token]);
  ...
  const submitFb = async () => {
    ...
    try {
      ...
      if (r.lote_fechado) { setLoteDone(true); setShowRate(true); }
      setObs(""); setShowObs(false); setSelFb(null);
      if (!r.lote_fechado) loadNext();
    } catch (e) { setMsg(e.message); }
    setFld(false);
  };
  const submitRate = async () => {
    if (rateNote > 0 && lead?.lista_id) {
      try { await sb.rpc("avaliar_lista", { p_lista_id: lead.lista_id, p_nota: rateNote }, token); } catch (e) { console.error(e); }
    }
    setShowRate(false); loadNext();
  };

  if (ld) return <div className="flex items-center justify-center h-64 text-gray-400">Carregando...</div>;

  if (showRate) return (
    <div className="p-4"><div className="bg-white rounded-2xl shadow-md p-5 border text-center">
      <div className="text-3xl mb-3">🎉</div><p className="font-bold text-emerald-800 mb-2">Lote completo!</p>
      <p className="text-sm text-gray-600 mb-4">Como você avalia a qualidade dessa lista?</p>
      <div className="flex justify-center mb-4"><Stars value={rateNote} onChange={setRateNote} /></div>
  ...
  if (!lead && !loteDone) return (
    <div className="flex flex-col items-center justify-center h-64 px-4"><div className="text-5xl text-gray-300 mb-4">◎</div><p className="text-gray-500 text-center">{msg || "Sem leads no momento."}</p><button className="mt-4 text-blue-600 text-sm font-medium" onClick={loadNext}>Verificar</button></div>
  );
  ...
  return (
    <div className="p-4 space-y-4">
      {prog && (<div className="px-0"><div className="flex justify-between text-xs text-gray-500 mb-1"><span>Lote</span><span className="font-medium">{prog.feitos}/{prog.total}</span></div><div className="w-full bg-gray-200 rounded-full h-3"><div className="bg-blue-600 h-3 rounded-full transition-all" style={{ width: (prog.feitos / prog.total * 100) + "%" }} /></div></div>)}
      {lead && (<div className="bg-white rounded-2xl shadow-md p-5 border border-gray-100">
        ...
        <div className="flex gap-3 mt-4">
          ...
          {lead.telefone_e164 && lead.tipo_telefone === "br_celular" && <a href={buildWhatsAppLink(lead)} target="_blank" rel="noopener" className="flex-1 bg-emerald-600 text-white rounded-xl py-4 text-center font-bold text-lg no-underline">WhatsApp</a>}
        </div>
      </div>)}
...
function ProducaoTab({ sb, token }) {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        <KPI label="Total recebidos" value={totais.total_recebidos || 0} />
        <KPI label="Com feedback" value={totais.com_feedback || 0} />
        <KPI label="Em carteira" value={totais.em_carteira || 0} color="text-blue-600" />
      </div>
      {chartData.length > 0 && (
      ...
      {data.com_observacao?.length > 0 && (
        <div><p className="text-sm font-bold text-gray-700 mb-2">Com observação</p><div className="space-y-2">{data.com_observacao.slice(0, 10).map((l, i) => (
          <div key={i} className="bg-white rounded-xl p-3 border border-gray-100 shadow-sm">
            ...
            {l.telefone && <div className="flex gap-2 mt-2"><a href={"tel:" + l.telefone} className="text-xs text-blue-600 no-underline">Ligar</a></div>}
          </div>
        ))}</div></div>
      )}
    </div>
...
function CarteiraTab({ sb, token }) {
  const [data, setData] = useState(null); const [ld, setLd] = useState(true);
  useEffect(() => { (async () => { try { const r = await sb.rpc("minha_producao", {}, token); setData(r); } catch (e) { console.error(e); } setLd(false); })(); }, []);
  if (ld) return <div className="p-4 text-center text-gray-400">Carregando...</div>;
        ...
        <div key={i} className="bg-white rounded-xl p-3 border border-gray-100 shadow-sm">
          <div className="flex justify-between items-start"><div><p className="font-medium text-sm text-gray-900">{l.nome}</p><p className="text-xs text-gray-500 mt-0.5">{l.telefone} · {l.feedback}</p></div>
            <div className="flex gap-2">
              ...
              {l.whatsapp && <a href={buildWhatsAppLink(l)} target="_blank" rel="noopener" className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded-lg no-underline">Zap</a>}
            </div>
          </div>
...
// ================================================================
// GESTOR: UPLOAD, DISTRIBUIR, DASHBOARD (mantidos da v1)
// ================================================================
function UploadTab({ sb, token }) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      for (let i = 0; i < leads.length; i += B) { const r = await sb.rpc("importar_leads_batch", { p_lista_id: lid, p_leads: JSON.stringify(leads.slice(i, i + B)) }, token); tot.validos += (r.validos || 0); tot.invalidos += (r.invalidos || 0); tot.duplicados += (r.duplicados || 0); }
...
function DistribuirTab({ sb, token }) {
  ...
  return (<div className="p-4 space-y-4"><h2 className="text-lg font-bold text-gray-900">Distribuir lotes</h2>{st && <div className="grid grid-cols-3 gap-3"><KPI label="Disponíveis" value={st.d} color="text-blue-600" /><KPI label="Lotes abertos" value={st.a} color="text-amber-600" /><KPI label="Corretores" value={st.c} color="text-emerald-600" /></div>}<button className="w-full bg-blue-600 text-white rounded-xl py-4 font-bold text-lg disabled:opacity-50" disabled={ld} onClick={go}>{ld ? "Distribuindo..." : "Distribuir agora"}</button>{result && !result.error && <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-200"><p className="font-bold text-emerald-800">{result.lotes_criados} lote(s) criado(s)</p></div>}{result?.error && <div className="bg-red-50 rounded-xl p-4 text-red-700 text-sm">{result.error}</div>}</div>);
}

// ================================================================
// GESTOR: DASHBOARD (atualizado com carteira e qualidade)
// ================================================================
...
// ================================================================
// GESTOR: LISTAS (gestão + relatório fornecedor)
// ================================================================
function ListasTab({ sb, token }) {
  const [listas, setListas] = useState([]); const [ld, setLd] = useState(true); const [report, setReport] = useState(null);
  const load = async () => { try { setListas(await sb.query("listas", "order=created_at.desc", token)); } catch (e) {} setLd(false); };
  useEffect(() => { load(); }, []);
  ...
  if (report) return (
    <div className="p-4 space-y-4">
      ...
      {report.avaliacoes?.length > 0 && (<div><p className="text-sm font-bold text-gray-700 mb-2">Avaliações dos corretores</p>{report.avaliacoes.map((a, i) => (<div key={i} className="bg-white rounded-lg p-3 border mb-2"><div className="flex justify-between"><span className="text-sm font-medium">{a.corretor}</span><Stars value={a.nota} /></div>{a.comentario && <p className="text-xs text-gray-500 mt-1">{a.comentario}</p>}</div>))}</div>)}
    </div>
  );
      ...
      {listas.map(l => (
        <div key={l.id} className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          ...
          <div className="flex gap-2 mt-3">
            ...
            {l.status !== "encerrada" && <button className="text-xs bg-red-100 text-red-700 px-3 py-1.5 rounded-lg" onClick={() => { if (confirm("Encerrar esta lista? Leads disponíveis serão invalidados.")) acao(l.id, "encerrar", "Baixa qualidade"); }}>Encerrar</button>}
          </div>
        </div>
      ))}
    </div>
...
function EquipeTab({ sb, token }) {
  const [cs, setCs] = useState([]); const [show, setShow] = useState(false); const [nn, setNn] = useState(""); const [ne, setNe] = useState(""); const [np, setNp] = useState(""); const [ng, setNg] = useState(false); const [ad, setAd] = useState(false); const [msg, setMsg] = useState("");
  const load = async () => { try { setCs(await sb.query("corretores", "order=nome.asc", token)); } catch (e) {} };
  useEffect(() => { load(); }, []);
  const add = async () => { setAd(true); setMsg(""); try { const r = await sb.signUp(ne, np); const uid = r.user?.id; if (!uid) throw new Error("Falha auth"); await sb.insert("corretores", { user_id: uid, nome: nn, email: ne, is_gestor: ng }, token); setMsg("Criado!"); setNn(""); setNe(""); setNp(""); setNg(false); setShow(false); load(); } catch (e) { setMsg("Erro: " + e.message); } setAd(false); };
  return (<div className="p-4 space-y-4"><div className="flex justify-between items-center"><h2 className="text-lg font-bold text-gray-900">Equipe</h2><button className="text-sm bg-blue-600 text-white px-3 py-1.5 rounded-lg" onClick={() => setShow(!show)}>+ Adicionar</button></div>
    {show && <div className="bg-gray-50 rounded-xl p-4 space-y-3"><input className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="Nome" value={nn} onChange={e => setNn(e.target.value)} /><input className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="Email" value={ne} onChange={e => setNe(e.target.value)} /><input className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="Senha (min 6)" type="password" value={np} onChange={e => setNp(e.target.value)} /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={ng} onChange={e => setNg(e.target.checked)} />Gestor</label><button className="w-full bg-blue-600 text-white rounded-lg py-2 font-medium disabled:opacity-50" disabled={ad || !nn || !ne || np.length < 6} onClick={add}>{ad ? "Criando..." : "Criar"}</button>{msg && <p className="text-sm text-center">{msg}</p>}</div>}
    <div className="space-y-2">{cs.map(c => (<div key={c.id} className="bg-white rounded-xl p-3 border shadow-sm flex justify-between items-center"><div><p className="font-medium text-sm">{c.nome} {c.is_gestor && <span className="text-xs bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full ml-1">Gestor</span>}</p><p className="text-xs text-gray-500">{c.email}</p></div><span className={`text-xs px-2 py-1 rounded-full ${c.ativo ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}>{c.ativo ? "Ativo" : "Inativo"}</span></div>))}</div></div>);
}
...
export default function App() {
  const [config, setConfig] = useState(null); const [session, setSession] = useState(null);
  const [corretor, setCorretor] = useState(null); const [loading, setLoading] = useState(true); const [sb, setSB] = useState(null);

  useEffect(() => { (async () => {
    try { const s = await window.storage.get("discador_config"); if (s?.value) { const c = JSON.parse(s.value); setConfig(c); setSB(createSB(c.url, c.key)); } } catch (e) {}
    try { const s = await window.storage.get("discador_session"); if (s?.value) setSession(JSON.parse(s.value)); } catch (e) {}
    setLoading(false);
  })(); }, []);

  useEffect(() => { if (!sb || !session) return; (async () => { try { const d = await sb.query("corretores", "user_id=eq." + session.user.id + "&select=*", session.access_token); if (d.length > 0) setCorretor(d[0]); } catch (e) { setSession(null); } })(); }, [sb, session]);

  const saveConfig = async (c) => { setConfig(c); setSB(createSB(c.url, c.key)); try { await window.storage.set("discador_config", JSON.stringify(c)); } catch (e) {} };
  const login = async (d) => { setSession(d); try { await window.storage.set("discador_session", JSON.stringify(d)); } catch (e) {} };
  const logout = async () => { setSession(null); setCorretor(null); try { await window.storage.delete("discador_session"); } catch (e) {} };
  ...
  if (!sb) return <div className="p-4 text-red-500">Erro de configuração</div>;
  if (!session) return <LoginScreen sb={sb} onLogin={login} />;
  if (!corretor) return <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4"><div className="bg-white rounded-2xl shadow-lg p-6 text-center max-w-sm"><p className="text-gray-700 mb-2">Usuário não encontrado na tabela de corretores.</p><p className="text-sm text-gray-500">Peça ao gestor para cadastrar seu email.</p><button className="mt-4 text-blue-600 text-sm font-medium" onClick={logout}>Voltar</button></div></div>;
