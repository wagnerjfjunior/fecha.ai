...
(function () {
  ...
  function updateDevelopmentGrid(grid) { const active = getDevelopment(); grid.querySelectorAll('[data-pme-inline-development]').forEach((btn) => { btn.classList.toggle('active', btn.getAttribute('data-pme-inline-development') === active); }); const note = grid.querySelector('.pme-inline-dev-note'); setText(note, `${DEVELOPMENTS[active]?.hint || ''} Endereço: ${DEVELOPMENTS[active]?.address || ''}`); }
  ...
  function start() { installProfileFetchBridge(); bind(); observer = new MutationObserver(() => schedulePatch()); patch(); reconnectObserver(); }
